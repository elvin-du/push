...
func AesEncryptString(key []byte, data string) (string, error) {
	bin, err := AesEncrypt(key, []byte(data))
	if err != nil {
		return "", err
	} else {
...
func AesDecryptString(key []byte, in string) (string, error) {
	bin, err := AesDecrypt(key, []byte(in))
	if err != nil {
		return "", err
	} else {
