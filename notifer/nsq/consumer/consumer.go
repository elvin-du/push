...
var (
	NSQLOOUPD_ADDRS = []string{"127.0.0.1:4161"}
...
func (n *nsqConsumer) Run() {
	...
	err = n.singleConsumer.ConnectToNSQLookupds(NSQLOOUPD_ADDRS)
	if nil != err {
		log.Fatalln(err)
	}
	...
	n.broadcastConsumer.ConnectToNSQLookupds(NSQLOOUPD_ADDRS)
	if nil != err {
		log.Fatalln(err)
	}
